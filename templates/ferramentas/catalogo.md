...
### Cloudflare Pages
**O que faz:** Publica páginas estáticas com link público (propostas, landing pages, estudos)
**Precisa de conta:** Sim, Cloudflare (grátis) + GitHub
**Configurar:** Conectado ao GitHub (repo: hurtzgestao-lang/hurtz-publicacoes)
**Status:** ✅ Configurado (domínio: pages.hurtzcompany.com.br)
**Estrutura:** Cada pasta em repositorio-publicacoes/ vira um slug (ex: teste/ → /teste/)
**Deploy:** Automático via git push no repo de publicações
**Quando usar:** Sempre que gerar HTML que precisa ser compartilhado por link
...
### OpenRouter (GPT Image / Gemini Flash)
**O que faz:** Gera imagens de alta qualidade via OpenRouter (acesso a GPT Image e Gemini Flash)
**Precisa de conta:** Sim, OpenRouter
**Configurar:** Salvar `OPENROUTER_API_KEY` no `.env`
**Status:** ✅ Configurado
**Quando usar:** Logos, ícones, ilustrações, imagens pra posts
...
**O que faz:** Dispara mensagens, avisa clientes, envia comunicação automática via WhatsApp
**Precisa de conta:** Sim, Evolution API (instância própria)
**Configurar:** Salvar `EVOLUTION_API_URL`, `EVOLUTION_API_KEY`, `EVOLUTION_INSTANCE` no `.env`
**Status:** ✅ Configurado (instância "Klebson")
**Quando usar:** Notificações, confirmações, follow-up com clientes
...
### Google Calendar API
**O que faz:** Gerencia eventos, agenda reuniões, consulta disponibilidade
**Precisa de conta:** Sim, Google Cloud + OAuth2
**Configurar:** Salvar `GOOGLE_REFRESH_TOKEN`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` no `.env`
**Status:** ✅ Configurado (2 calendários conectados)
**Quando usar:** Agendamento, organização de reuniões, consulta de agenda

### Google Sheets API
**O que faz:** Lê, escreve e manipula planilhas Google Sheets
**Precisa de conta:** Sim, Google Cloud + OAuth2
**Configurar:** Usa as mesmas credenciais do Calendar (inclui scope `spreadsheets` e `drive.readonly`)
**Status:** ✅ Configurado (20 planilhas acessíveis)
**Quando usar:** Relatórios, bases de leads, métricas, manipulação de dados
