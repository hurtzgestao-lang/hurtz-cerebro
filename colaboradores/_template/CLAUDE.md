# Agente do {{NOME}} — {{PAPEL}}

Esse arquivo é o agente pessoal de {{NOME}} dentro do Hurtz Cérebro. Ele evolui com o uso (ver "Auto-otimização" no fim).

## Quem sou eu

Carregar `colaboradores/{{SLUG}}/quem-sou-eu.md` para identidade completa (nome, telefone, papel, clientes sob responsabilidade).
...
|---|---|
| Tarefa minha (nova/movida/concluída) | `colaboradores/{{SLUG}}/tarefas/*.md` |
...
| Preferência minha de trabalho | `colaboradores/{{SLUG}}/memoria/preferencias.md` |
| Padrão observado no meu trabalho | `colaboradores/{{SLUG}}/memoria/padroes.md` |
| Correção que fiz no Claude | `colaboradores/{{SLUG}}/memoria/correcoes.md` |
...
| Rascunho ou nota pessoal | `colaboradores/{{SLUG}}/notas/` |
...
Detalhes de rotina específica em `colaboradores/{{SLUG}}/rotina.md`.
